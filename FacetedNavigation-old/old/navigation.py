...
class Navigation(object):
    ...
    def changeY(self):
        ...
        if self.z_id==-1:
            for i in range(table.column_num):
                if table.types[i]==Type.numerical and table.origins[i]!=self.y_id:
    ...
    def changeNormalBin(self):
        bin=self.describe.split()[4]
        if bin=='TIME':
            self.changeToNormalBin('WEEKDAY')
            if type(self.instance.tables[0].features[self.instance.ids[self.x_name]])!=type(datetime.date(1995, 10, 11)):
